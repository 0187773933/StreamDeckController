...
RUN apt-get install libhidapi-libusb0 -y
...
RUN echo 'SUBSYSTEMS=="usb", ATTRS{idVendor}=="0fd9", GROUP="users", TAG+="uaccess"' | tee /etc/udev/rules.d/10-streamdeck.rules
...
RUN python -m pip install wheel -U
